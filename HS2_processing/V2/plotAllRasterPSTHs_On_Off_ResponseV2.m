...
nBlocks  = length(responseMetrics.trialSpikes);
for stimblk = 1:nBlocks
    ...
    if ~isnan(responseMetrics.TI_on(stimblk))
        TIon_numeric = num2str(responseMetrics.TI_on(stimblk));
    else
        TIon_numeric = 'NAN';
    end

     if ~isnan(responseMetrics.TI_off(stimblk))
        TIoff_numeric = num2str(responseMetrics.TI_off(stimblk));
    else
        TIoff_numeric = 'NAN';
    end
    title([titleText{stimblk} ' ON TI: ' TIon_numeric ' OFF TI: '  TIoff_numeric ' QI: ' num2str(responseMetrics.responseQuality(stimblk))  ' ON Z-score: ' num2str(responseMetrics.ZValueON(stimblk))  ' OFF Z-score: ' num2str(responseMetrics.ZValueOFF(stimblk))]);
