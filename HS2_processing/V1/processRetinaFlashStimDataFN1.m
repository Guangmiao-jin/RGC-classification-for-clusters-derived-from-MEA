function data = processRetinaFlashStimDataFN1(clusterFilepath, prestimTime, stimTime, spikeTotalLower)
    ...
    % if any(zScorePerClusterBlkON(i,:)  >zScoreLower) || any(abs(zScorePerClusterBlkOFF(i,:))  >zScoreLower)
        if data.channelNames{6,i} >= spikeTotalLower
            ...
            if length(stimOnPerBlock) ~= 1
                ...
                ax = plotAllRasterPSTHs_On_Off_Response(spikeTimes, binsize, data.Sampling, stimOnPerBlock, stimOffPerBlock, 0.5, stimTime);                
            end
            ...
            if length(stimOnPerBlock) ~= 1
                ...
                ax = ax(arrayfun(@(a) isa(a, 'matlab.graphics.axis.Axes'), ax));
                subplotEvenAxes(ax, [0 1 0] , 3:4);
            end
            tightfig;
